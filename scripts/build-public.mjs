...
await build({entryPoints:['web/app.mjs'],bundle:true,format:'esm',splitting:true,outdir:'dist-public',entryNames:'app',chunkNames:'chunks/[name]-[hash]',target:['es2022'],minify:true,define:{PUBLIC:'true'},metafile:true,plugins:[{name:'isolate-public-chain',setup(b){b.onLoad({filter:/web\/app\.mjs$/},async args=>({contents:(await readFile(args.path,'utf8')).replace(/^const PUBLIC=.*;\n/,''),loader:'js'}));b.onResolve({filter:/^\.\/chain\.mjs$/},()=>({path:new URL('../web/chain-public.mjs',import.meta.url).pathname}));}}]}).then(r=>writeFile('dist-public/metafile.json',JSON.stringify(r.metafile,null,2)));
for(const file of ['index.html','style.css','favicon.svg'])await copyFile(`web/${file}`,`dist-public/${file}`);
